...
# NOTE (bugfix): results/ (and results/figure/ under it) is gitignored, so
# it doesn't exist on a fresh clone. fig.savefig() below used to fail with
# FileNotFoundError the first time anyone ran this script, since nothing
# ever created the folder first.
...
# Same analysis as before, just run once per observable instead of being
# hardcoded to |m| -- the column names in L{L}_tau.csv already follow the
# tau_<obs>/err_tau_<obs> convention (tau.py writes both tau_abs_m and
# tau_E), so looping over this list is the only new thing; everything below
# is untouched, just indented one level and reading obs_col/obs_tex instead
# of the literal "abs_m"/"|m|".
observables = [("abs_m", r"|m|"), ("E", r"E")]

for obs_col, obs_tex in observables:
    ...
    for i in range(0,metadata.shape[0]) :
        ...
        tau[i] = data[f"tau_{obs_col}"][idx]
        err_tau[i] = data[f"err_tau_{obs_col}"][idx]
    ...
    ax.set_ylabel(fr"$\log \tau_{{int}}^{{{obs_tex}}}$")
    ...
    axins.set_ylabel(fr"$\tau_{{int}}^{{{obs_tex}}}$",size=16)

    ax.legend()
    fig.tight_layout()
    fig.savefig(f"results/figure/{sim_name}_dynamic_z_{obs_col}.pdf",bbox_inches="tight")
    ...
    if alg == "wolff" :
        ...
        ax2.set_xlabel(r"$L$")
        ax2.set_ylabel(fr"$\tau_{{int}}^{{{obs_tex}}}$")
        ...
        ax2.legend()
        fig2.tight_layout()
        fig2.savefig(f"results/figure/{sim_name}_dynamic_z_log_{obs_col}.pdf",bbox_inches="tight")
