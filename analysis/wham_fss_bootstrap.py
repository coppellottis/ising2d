...
# =============================================================================
# FIT FSS: beta_pc(L) = beta_c + x0 * L^(-1/nu)
# =============================================================================
def beta_max_fit(L, beta_c, x0, nu):
    return beta_c + x0 * np.asarray(L, dtype=float)**(-1/nu)

# Jacobiano analitico di beta_max_fit rispetto a (beta_c, x0, nu), usato per
# propagare la covarianza del fit (pcov di curve_fit) sulla banda d'errore
# del grafico -- generalizza il calcolo che in binder.py era scritto a mano
# (var_fit = pcov[0,0] + x^2*pcov[1,1] + 2*x*pcov[0,1]) al caso non lineare
# a 3 parametri: qui il "coefficiente" di ciascun parametro nella somma
# quadratica non e' piu' una potenza fissa di x, va derivato per ognuno.
# Verificato contro un jacobiano numerico a differenze finite (differenza
# massima ~1e-11 su un caso di prova) prima di usarlo nel grafico.
def beta_max_fit_jac(L, beta_c, x0, nu):
    L = np.asarray(L, dtype=float)
    Lpow = L**(-1/nu)
    d_beta_c = np.ones_like(L)
    d_x0 = Lpow
    d_nu = x0 * Lpow * np.log(L) / nu**2
    return np.stack([d_beta_c, d_x0, d_nu], axis=-1)  # shape (n_L, 3)

# Banda d'errore sul fit: sigma_fit(L) = sqrt(J(L) . pcov . J(L)^T), la
# stessa quantita' di binder.py (var_fit lì sopra) ma generale per un
# numero qualsiasi di parametri/un modello qualsiasi, dato il suo jacobiano.
def fit_error_band(L_grid, popt, pcov, jac_func):
    J = jac_func(L_grid, *popt)
    var_fit = np.einsum('ij,jk,ik->i', J, pcov, J)
    return np.sqrt(np.maximum(var_fit, 0.0))
...
# =============================================================================
def main():
    # stile grafico -- stesso blocco usato in binder.py/wham_C.py
    plt.rcParams.update({
        "font.family": "serif",
        "mathtext.fontset": "cm",
        "font.size": 18,
    ...
    # Fit "di riferimento" su tutte le taglie -- usato per il grafico sotto.
    # E' lo stesso identico fit che il loop di stabilita' qui sotto rifa' da
    # capo a i=0 (L_min = L piu' piccola); tenerlo separato evita di dover
    # tirare fuori popt/pcov da dentro il loop e non cambia nulla nel loop
    # stesso, che resta quello che avevi scritto.
    popt_full = pcov_full = None
    try:
        popt_full, pcov_full = curve_fit(
            beta_max_fit,
            L_array, beta_max_mean_arr,
            p0=[0.4407, 1, 1],
            sigma=beta_max_err_arr,
            absolute_sigma=True,
            maxfev=10000
        )
    except Exception as e:
        print(f"Fit completo (tutte le {len(L_array)} taglie) non convergente, "
              f"niente grafico: {e}")
    ...
    # =========================================================================
    # GRAFICO: beta_max(L) vs L, fit e banda d'errore (stile di binder.py)
    # =========================================================================
    if popt_full is not None:
        beta_c_full, x0_full, nu_full = popt_full
        err_beta_c_full = np.sqrt(pcov_full[0, 0])

        L_grid = np.linspace(L_array.min(), L_array.max(), 200)
        y_grid = beta_max_fit(L_grid, *popt_full)
        sigma_grid = fit_error_band(L_grid, popt_full, pcov_full, beta_max_fit_jac)

        fig, ax = plt.subplots(figsize=(12, 8))

        ax.errorbar(L_array, beta_max_mean_arr, yerr=beta_max_err_arr,
                    fmt='o', markersize=5, capsize=3, color='black', label='Dati (bootstrap)')

        ax.plot(L_grid, y_grid, label=fr'Fit: $\beta_c={format_error(beta_c_full, err_beta_c_full)}$')
        ax.fill_between(L_grid, y_grid - sigma_grid, y_grid + sigma_grid, alpha=0.2)

        ax.set_xscale("log")
        ax.set_xlabel(r"$L$")
        ax.set_ylabel(r"$\beta_{pc}(L)$")

        ax.legend(loc="best")
        
        fig.savefig(f"results/figure/{sim_name}_beta_pc_fss.pdf", bbox_inches="tight")
        plt.show()
    else:
        print("Nessun grafico prodotto: il fit su tutte le taglie non e' convergente (vedi sopra).")
